...
decodeHtmlEnt = (text)->
  text.replace(/&#?(\w+);/g, (match, dec) ->
    if isNaN(dec)
      if CHARS[dec]?
        digi = CHARS[dec]
    String.fromCharCode(digi)
  )
...
class ChannelInfo
    constructor: (props) ->
      { @name, @id,  @address,
        @contact_url, @genre, @detail, @viewer, @relay,
        @bitrate, @file_type, @channel_name, @broadcast_time,
        @status, @comment } = props

    @from_line: (line) ->
      a = line.split('<>')
      new ChannelInfo({
        name: a[0], id: a[1], address: a[2], contact_url: a[3], genre: a[4]
        detail: decodeHtmlEnt(a[5]), viewer: a[6], relay: a[7], bitrate: a[8], file_type: a[9]
        channel_name: decodeURI(a[14]), broadcast_time: a[15],
        status: a[16], comment: a[17]
      })
...
if require.main == module
  ...
  request yp, (error, response, body) ->
